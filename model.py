...
class UNet(nn.Module):
    ...
    def frequency_filter(self, frequency_maps, type="low", threshold=0.48):
        if type == "low":
            # Apply low-pass filter (keep low frequencies)
            return frequency_maps * (
                abs(frequency_maps) < threshold
            )  # custom threshold
        elif type == "high":
    ...
    def fuse_features(self, x, low_freq, high_freq, channel_size):
        conv = nn.Conv2d(channel_size, channel_size // 2, kernel_size=3, padding=1).to(
            x.device
        ...
        conv1 = nn.Conv2d(channel_size * 2, channel_size, kernel_size=3, padding=1).to(
            x.device
        )  # Ensure on the same device
